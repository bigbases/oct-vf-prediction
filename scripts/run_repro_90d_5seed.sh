#!/usr/bin/env bash
# 90d repro: 9:1 × 5 seeds (42–46), image-only + multimodal
set -euo pipefail
...
MASTER_LOG=runs/repro_90d_5seed_seq_flip_lr1e4_p100.log

log() { echo "=== $* ==="; }

log "Repro 90d 5-seed 시작 $(date)"
...
"$PY" -u scripts/summarize_repro_5seed.py \
  ...
  --out runs/repro_90d_5seed_summary.json \
  2>&1 | tee -a "$MASTER_LOG"

log "Repro 90d 5-seed 전체 완료 $(date)"
